...
#' 
#' @export
sarima2ar <- function(x, xreg = NULL, period = 1, tol = 500L, 
  ...
  if (sum(order) == 0L & sum(seasonal) == 0L){
    ...
    fit <- forecast::auto.arima(x, xreg = xreg, allowdrift = allowdrift, allowmean = allowmean, 
    ...
    fit <- forecast::Arima(x, xreg = xreg, order = order, seasonal = seasonal, method = method, 
  ...
  if (isEmpty(dft))
    dft <- 0
  reg_coefs <- fit$coef[substring(names(fit$coef), 1, 4) == "xreg"]
  ...
  # return(list(coef = ar.coef, sigma2 = sigma2, reg.coef = reg.coef))
